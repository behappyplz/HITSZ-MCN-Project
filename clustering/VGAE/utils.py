...
def mask_test_edges(adj):
    ...
    # Re-build adj matrix (my)
    data = np.ones(edges.shape[0])
    adj_all = sp.csr_matrix((data, (edges[:, 0], edges[:, 1])), shape=adj.shape)
    adj_all = adj_all + adj_all.T

    # NOTE: these edge lists only contain single direction of edge!
    return adj_train, train_edges, val_edges, val_edges_false, test_edges, test_edges_false, adj_all

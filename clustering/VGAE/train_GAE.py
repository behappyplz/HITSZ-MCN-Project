        ...
        # update_interval
            with torch.no_grad():
                ...
                if epoch==60:
                    ...
                    plt.savefig("/home/sunyang/MCN_main/figures/gae_umap_wo.png")
        t = time.time()
        model.train()
        optimizer.zero_grad()
        recovered, mu = model(features, adj_norm)
        loss = loss_function(preds=recovered, labels=adj_label,
                             mu=mu, logvar=None, n_nodes=n_nodes,
                             norm=norm, pos_weight=pos_weight,gae=True)
        loss.backward()
        cur_loss = loss.item()
        optimizer.step()
